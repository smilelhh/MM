...
namespace MM
{
    public partial class FixApplication : Form
    {
        ...
        private void FixApplication_Load(object sender, EventArgs e)
        {
           ...
           // this.t_InUsingTableAdapter.Fill(this.bYMMDataSetFixApplication.T_InUsing);
        ...
        {
            DateTime applyFix_date = new DateTime();
            applyFix_date = Convert.ToDateTime(tbmdate.Text.Trim());
            float mPrice = 0;
            ...
            string connString = "server=172.168.16.235;Initial Catalog=BYMM;uid=sa;pwd=123456";
            SqlConnection conn = new SqlConnection(connString);
            string sqlString = "insert into T_ApplyFix values('" + cb_mnum.Text.Trim() + "','" + tbmname.Text.Trim() + "','" + tbmcategory.Text.Trim() + "'," + mPrice + ",'" + applyFix_date + "','" + tbapname.Text.Trim() + "','" + tbapnum.Text.Trim() + "','" + tbaptel.Text.Trim() + "','" + tbapdep.Text.Trim() + "','" + tbapdeptel.Text.Trim() + "','" + tbmagent.Text.Trim() + "')";
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(sqlString, conn);
                cmd.ExecuteNonQuery();
                MessageBox.Show("恭喜，申请修理成功", "友情提示");

            }
            catch (Exception)
            {
                MessageBox.Show("!插入修理表失败", "友情提示");
            }
            finally
            {
                conn.Close();
